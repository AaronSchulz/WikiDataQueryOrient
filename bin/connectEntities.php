...
function iterateEntities( WdqUpdater $updater, $class, $bSize, $posFile, callable $callback ) {
	...
	do {
		...
		$res = $updater->tryQuery(
			"SELECT id,claims,deleted,@rid,'$type' AS type FROM $class " .
			"where id > $pos ORDER BY id", $bSize );
		if ( !$res ) {
			break;
		}
...
function main() {
	...
	$bSize = 100;
	iterateEntities( $updater, 'Property', $bSize, null,
		function( $entities, $firstBatch ) use ( $updater, $method, $classes ) {
			...
			print( "Importing edges for Properties $from-$to ($safeMethod)..." );
			$updater->makeEntityEdges( $entities, $method, $classes );
			print( "done\n" );
			return true;
		}
	);
	...
	$bSize = 100;
	iterateEntities( $updater, 'Item', $bSize, $posFile,
		function( $entities, $firstBatch ) use ( $updater, $method, $classes ) {
			// Restarting might redo the first batch; preserve idempotence
			$safeMethod = $firstBatch ? 'rebuild' : $method;
			$from = $entities[0]['id'];
